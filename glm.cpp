...
vector<int> boxCoordinate;//´¢´æÄ£ÐÍboxµÄÄÚÈÝ
...
//ÉèÖÃÅÐ¶Ï±Ç¼âµãµÄboxÇøÓò£¬¼´È¡Öµ·¶Î§¡£
...
int glmNosePoint(GLMmodel* model){
	...
	//·Ö±ð¼ÆËã×î´óºÍ×îÐ¡µÄx£¬y£¬zÖµµÄµã
	//return i_maxx;
	//return i_minx;
	//return i_maxy;
	//return i_miny;
...
vector<vector <int>> glmPointNeibor(GLMmodel* model){
	...
	//printf("glmPointNeibor:%dµÄÁÚ¾ÓÎª:\n",3300);
	//for (int i = 0; i < temp.at(3300).size(); i++){
		//printf("%d\n", temp.at(3300).at(i));	
	//}
	//´Ë´¦¶ÔtempÈ¥ÖØ
	for (int i = 0; i < temp.size(); i++){
		vector<int> neibor_i = temp.at(i);
		vector<int> neibor_i_new;
		for (int j = 0; j < neibor_i.size(); j++){
			int index_j = neibor_i.at(j);
			if (!glmPointNeibor_pointexist(index_j, neibor_i_new)){
				neibor_i_new.push_back(index_j);			
			}
		}	
		result.push_back(neibor_i_new);
	}
	return result;
}

bool glmPointNeibor_pointexist(int point, vector<int> data){
...
int glmIndexPoint(GLMmodel* model, double x, double y){
	...
	double yt = model->vertices[3 * 1 + 1];	
	double zt = model->vertices[3 * 1 + 2];
	double minLength = (x - xt)* (x - xt) + (y - yt)* (y - yt);
	for (int i = 1; i <= model->numvertices; i++){
		xt = model->vertices[3 * i];
		yt = model->vertices[3 * i + 1];		
		double length = (x - xt)* (x - xt) + (y - yt)* (y - yt);
		if (length < minLength){
			indexPoint = i;
			minLength = length;
		}
	...
	//cout << "x=" << x << "y=" << y <<endl;	
	//cout << "xt = " << model->vertices[3 * indexPoint] << "yt=" << model->vertices[3 * indexPoint + 1] << "zt=" << model->vertices[3 * indexPoint + 2] << endl;
	//cout << indexPoint << endl;
	return indexPoint;
}

vector<vector<double>> glmTriangle(GLMmodel* model, vector<vector<int>> pointsIndex){
	vector<vector <int>> NeiborStructure = glmPointNeibor(model);
	vector<vector<vector<int>>> trangleNum;
	vector<vector<double>> trangleCord;
	for (int i = 0; i < pointsIndex.size(); i++){
		
		vector<int> pointsIndex_i = pointsIndex.at(i);//µÚiÌõ²âµØµØ´ø	
		vector<vector<int>> trangleNum_i;//µÚiÌõ²âµØµØ´ø¼ÇÂ¼Èý½ÇÐÎ
		for (int j = 0; j < pointsIndex_i.size(); j++){
			int firstPoint = pointsIndex_i.at(j);
			//cout << "firstPoint:" << firstPoint << endl;
			vector<int> firstPointNeoborHaveProcess;
			vector<int> firstPointNeobor = NeiborStructure.at(firstPoint);
			for (int k = 0; k < firstPointNeobor.size(); k++){
				int secondPoint = firstPointNeobor.at(k);
				if (glmPointNeibor_pointexist(secondPoint, firstPointNeoborHaveProcess)){
					continue;				
				}
				if (!glmPointNeibor_pointexist(secondPoint, pointsIndex_i)){
					continue;
				}
				vector<int> secondPointNeobor = NeiborStructure.at(secondPoint);
				for (int l = 0; l < secondPointNeobor.size(); l++){
					int thirdPoint = secondPointNeobor.at(l);

					if (glmPointNeibor_pointexist(thirdPoint, firstPointNeoborHaveProcess) || thirdPoint == firstPoint){
						continue;
					}
					if (!glmPointNeibor_pointexist(thirdPoint, pointsIndex_i)){
						continue;
					}
					if (glmPointNeibor_pointexist(thirdPoint, firstPointNeobor) &&
						glmPointNeibor_pointexist(thirdPoint, secondPointNeobor)){
						vector<int> trangleNum_ij;
						trangleNum_ij.push_back(firstPoint);
						trangleNum_ij.push_back(secondPoint);
						trangleNum_ij.push_back(thirdPoint);
						trangleNum_i.push_back(trangleNum_ij);
					}				
				}
				firstPointNeoborHaveProcess.push_back(secondPoint);
			
			}
		
		}
		trangleNum.push_back(trangleNum_i);	
	}

	for (int i = 0; i < trangleNum.size(); i++){

		vector<vector<int>> trangleNum_i = trangleNum.at(i);//µÚiÌõ²âµØµØ´øÖÐµÄÈý½ÇÃæÆ¬
		vector<double> trangleNum_cord_i;
		for (int j = 0; j < trangleNum_i.size(); j++){
			vector<int> trangleNum_ij = trangleNum_i.at(j);
			int b0 = trangleNum_ij.at(0);
			int b1 = trangleNum_ij.at(1);
			int b2 = trangleNum_ij.at(2);

			double x0 = model->vertices[3 * b0];
			double y0 = model->vertices[3 * b0 + 1];
			double z0 = model->vertices[3 * b0 + 2];

			double x1 = model->vertices[3 * b1];
			double y1 = model->vertices[3 * b1 + 1];
			double z1 = model->vertices[3 * b1 + 2];

			double x2 = model->vertices[3 * b2];
			double y2 = model->vertices[3 * b2 + 1];
			double z2 = model->vertices[3 * b2 + 2];

			trangleNum_cord_i.push_back(x0);
			trangleNum_cord_i.push_back(y0);
			trangleNum_cord_i.push_back(z0);

			trangleNum_cord_i.push_back(x1);
			trangleNum_cord_i.push_back(y1);
			trangleNum_cord_i.push_back(z1);

			trangleNum_cord_i.push_back(x2);
			trangleNum_cord_i.push_back(y2);
			trangleNum_cord_i.push_back(z2);

		
		}
		trangleCord.push_back(trangleNum_cord_i);
	
	}
	cout << "ÃæÆ¬Éú³ÉÍê±Ï£¡" << endl;
	return trangleCord;
}
